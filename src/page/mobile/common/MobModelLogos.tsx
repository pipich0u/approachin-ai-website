...
export const MobModelLogos = () => {
    return (
        <div className="mob-logos-container">
            <div className="mob-logos-track">
                {[...logos, ...logos].map((item, index) => (
                    <div className="mob-logos-item" key={index}>
                        <img src={item.src} alt={item.name} className="mob-logos-icon" loading="lazy" />
                        <span className="mob-logos-name">{item.name}</span>
                    </div>
                ))}
            </div>
        </div>
