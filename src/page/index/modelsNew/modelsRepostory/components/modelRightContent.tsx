...
const ModelRightContent = (props: ModelRightContentProps) => {
    const { showModal, openApi, onSearch, modelList, loadMore, page } = props;
    ...
    return <div className='flex-1 p-[18px] box-border w-[calc(100%-280px)] h-full'>
        <div className="model-content-right flex  items-center justify-between w-full mb-5">
            <div className='text-t flex'>
                <div>模型数</div>
                <div className='text-[#ccc]'>&nbsp;|&nbsp;{page.total}个</div>
            </div>
            <div>
        ...
        <div className='overflow-y-auto h-[calc(100%-70px)] scrollbar'>
            {
                modelList.length > 0 ? <div className='flex flex-wrap gap-5'>
                    {
                        modelList.map((item) => {
                ...
                </div> : <div><Empty description="暂无数据" /></div>
            }
            {page.current * page.pageSize < page.total && <div className='flex items-center justify-center mt-8 text-[12px] font-[330]  text-[#666666]' >
